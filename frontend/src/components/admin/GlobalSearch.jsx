  ...
  return <div className="relative" ref={boxRef}>
    {open
      ? <div className={`flex items-center gap-2 rounded-full px-3 py-2 ${isLight ? 'border border-kBorder bg-kCream' : 'bg-white/10'}`}>
          ...
          <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Search members, volunteers, visits, donations…" className={`w-40 bg-transparent text-sm focus:outline-none sm:w-72 ${isLight ? 'text-kInk placeholder:text-kMuted' : 'text-white placeholder:text-white/40'}`} />
          <button onClick={close} className={isLight ? 'text-kMuted hover:text-kInk' : 'text-white/60 hover:text-white'}><X size={15} /></button>
        </div>
      : isLight
        ? <button onClick={() => setOpen(true)} className="hidden items-center gap-2 rounded-full border border-kBorder bg-kCream px-4 py-2 text-sm text-kMuted transition hover:border-kGreen hover:text-kInk sm:flex sm:w-56 md:w-80" aria-label="Search">
            <Search size={15} /> <span className="truncate">Search members, volunteers, visits, donations…</span>
          </button>

...
export default function ReportConcern({ showToast }) {
  ...
  return <VolunteerShell>
    <PageHeader eyebrow="Safeguarding" title="Report a Concern" subtitle="Raise a medical, safety, welfare or other concern directly with KDCCE staff. Critical concerns notify staff immediately." />

    {submitted ? <div className="card-k mt-7 p-9 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-kSuccess/10 text-kSuccess"><CheckCircle2 /></div><h2 className="mt-4 font-display text-xl font-bold text-kGreen">Concern reported</h2><p className="mt-2 text-sm text-kMuted">KDCCE staff have been notified and will follow up as needed.</p><button onClick={() => setSubmitted(false)} className="btn-orange mt-6">Report another concern</button></div> : <form onSubmit={submit} className="card-k mt-7 grid gap-4 p-6">

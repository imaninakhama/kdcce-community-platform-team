...
VOLUNTEER_HOURS_CATEGORIES = ("Home Visit", "Assistance", "Activity", "Administrative", "Other")
VOLUNTEER_HOURS_STATUSES = ("Pending", "Approved", "Rejected")
ACHIEVEMENT_CATEGORIES = ("Hours", "Visits", "Assistance", "Recognition")
...
# type is a deterministic threshold achievements/service.py evaluates
# against real computed data.
ACHIEVEMENT_THRESHOLD_TYPES = ("service_minutes", "completed_visits", "completed_assistance", "completed_assignments", "manual")
...
class User(db.Model):
    ...
    # --- admin ops: users/sessions/2FA -----------------------------------
    active = db.Column(db.Boolean, nullable=False, default=True)
    last_login_at = db.Column(db.DateTime(timezone=True))
    # Soft delete for users only — a deleted account is excluded from
    ...
    # Base32, written once at 2FA setup and read only by auth/totp_service —
    # never included in to_dict() output.
...
class VolunteerHours(db.Model):
    ...
    app/volunteer_hours/service.py). Only Approved rows count toward any
    total; Pending/Rejected never do. Minutes, not floating-point hours —
...
class Achievement(db.Model):
    ...
    per-volunteer record (VolunteerAchievement below is that). Seeded
    once with a fixed starter set (see migration + app/achievements/
    seed_data.py); no admin-facing create/edit API."""
...
class VolunteerAchievement(db.Model):
    ...
    award (achievements/service.py's checker, awarded_by_id null) from a
    manual recognition award (awarded_by_id is the acting admin/staff
    user). At most once per volunteer per achievement, for both kinds."""
...
class AuditLog(db.Model):
    """A minimal, general-purpose audit trail via the single
    app/audit/service.py:log_action() chokepoint, same "one function
    every call site goes through" shape as notifications.service.notify.
    before/after are JSON-serialized snapshots of only the fields that
    changed — never a raw request body."""
...
class UserSession(db.Model):
    ...
    RevokedToken (a denylist of what's dead, not a record of what's
    currently active). Revoking a session here also inserts a
    RevokedToken row for the same refresh_jti (see app/sessions/
    service.py), so the two can never drift apart."""
...
class TwoFactorChallenge(db.Model):
    ...
    valid on any jwt_required() route the moment it's minted). Real
    access/refresh tokens are only minted after this is redeemed via
    /2fa/verify-login or /2fa/recovery."""
...
class TotpRecoveryCode(db.Model):
    ...
    werkzeug generate/check_password_hash used for account passwords),
    never in plaintext. The plaintext is shown to the user exactly once,
    at enrollment confirmation, and never again."""

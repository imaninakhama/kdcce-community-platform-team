...
def test_self_update_rejects_invalid_emergency_contact_phone(client, make_user, auth_header):
    _, access_token, _ = make_user(email="vol_badecphone@example.com")
    resp = client.patch("/api/volunteers/me", json={"emergency_contact_phone": "999"}, headers=auth_header(access_token))
    assert resp.status_code == 400
